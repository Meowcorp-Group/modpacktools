...
cwd = Path.cwd() / 'modpacktools' / 'server'
...
def getMods(data: dict) -> ResourceList:
	...
	for file in data['files']:
		...
		# if path starts with `resourcepacks` or `shaderpacks`, skip
		if path.startswith('resourcepacks') or path.startswith('shaderpacks'):
			print('Skipping:', path)
			continue

		# if env['server'] is `unsupported`, skip
...
def main() -> None:
	print('download_server: download server files from mrpack')
